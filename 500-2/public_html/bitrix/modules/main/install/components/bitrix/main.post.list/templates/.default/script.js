...
*/
	...
	BX.addCustomEvent(window, 'BX.Forum.Spoiler:toggle', function(params) {
		FCList.onForumSpoilerToggle(params);
	});
	BX.ready(function() {
		setTimeout(function() {
			FCList.recalcMoreButtonCommentsList();
	...
	window.FCList.onForumSpoilerToggle = function(params)
	{
		...
		var outerBlock = BX.findParent(params.node, { className: 'feed-com-text'});
		if (outerBlock)
		{
			var bodyBlock = BX.findChild(outerBlock, { className: 'feed-com-text-inner-inner'}, true);
			if (bodyBlock)
			{
				FCList.recalcMoreButtonComment({
					bodyBlock: bodyBlock,
