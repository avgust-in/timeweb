;(function() {
	...
	BX.Filter.Fields.prototype = {
		...
		createTextarea: function(fieldData)
		{
			var field = BX.decl({
				block: 'main-ui-control-field',
				mix: this.parent.getParam('ENABLE_LABEL') ? [this.parent.settings.classFieldWithLabel] : null,
			...
			var onChange = function() {
				BX.style(textarea, 'height', '1px');
				BX.style(textarea, 'height', textarea.scrollHeight + 'px');
			};

			BX.bind(textarea, 'input', onChange);
			BX.bind(textarea, 'change', onChange);
			BX.bind(textarea, 'keyup', onChange);
			BX.bind(textarea, 'cut', onChange);
			BX.bind(textarea, 'paste', onChange);
